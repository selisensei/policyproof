...
export function ReviewFingerprintPanel({ fingerprint, comparison, results, candidateResults, isVerifying, onRerun, compact = false }: {
  ...
  return (
    <section className="review-fingerprint" data-state={comparison?.kind ?? "READY"} data-compact={compact || undefined} aria-labelledby={compact ? "focused-fingerprint-title" : "workspace-fingerprint-title"}>
      <header>
        <div>
          <p className="eyebrow">SHA-256 · {locale === "fr" ? "REPRODUCTIBILITÉ" : "REPRODUCIBILITY"}</p>
      ...
      <p>{locale === "fr" ? "Empreinte calculée à partir des entrées et conclusions." : "Fingerprint generated from review inputs and conclusions."}</p>
      <div className="fingerprint-actions">
        <button type="button" className="primary-button" onClick={onRerun} disabled={isVerifying}>{isVerifying ? (locale === "fr" ? "Vérification…" : "Verifying…") : (locale === "fr" ? "Relancer les contrôles déterministes" : "Rerun deterministic checks")}</button>
      ...
      {comparison?.kind === "DIVERGED" && (
        <div className="fingerprint-verification" role="alert" data-tone="error">
          ...
          <ul>{comparison.changedControlIds.map((controlId) => {
            const previous = results.find((result) => result.controlId === controlId)?.status ?? "—";
            const candidate = candidateResults.find((result) => result.controlId === controlId)?.status ?? "—";

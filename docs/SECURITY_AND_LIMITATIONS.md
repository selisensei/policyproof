...
An unchanged Review Fingerprint supports byte-level equality of the normalized semantic review payload supplied to SHA-256. It excludes human decisions and receipt metadata. The separate Receipt Integrity Hash covers one receipt instance, including decisions, comments, safe audit metadata, language, identifier, and generation timestamp. Local verification recalculates the unkeyed hash without a network request. Neither digest proves that an original document is authentic, who created or approved the review, when it existed, or whether it is legally binding. Neither is a digital signature, trusted timestamp, fraud guarantee, or compliance certification. A person able to replace both receipt content and its stored unkeyed hash can create a new internally consistent digest.

## Evaluation and adversarial boundaries

The local Competition Evaluation Harness blocks fetch and Node HTTP/HTTPS access for its execution scope and records zero external calls on success. It tests three controlled fictional scenarios, seven business-rule mutations, strict threshold boundaries, and ten named hostile or malformed inputs. Prompt-like and script-like source text remains untrusted data in the tested deterministic and React rendering boundaries; fabricated excerpts, unknown documents, wrong relationships, duplicate identities, ambiguous numbers/dates, and forbidden receipt fields fail closed.

These results do not prove universal prompt-injection resistance, absence of all application vulnerabilities, complete policy coverage, source authenticity, or generalized model accuracy. No penetration test or external benchmark was performed.

**EXPECTED SECURITY BOUNDARY:** receipt content with its original recorded hash can be checked and modifications retained with the old hash are detected. A person able to replace both content and the unkeyed hash can create a new internally consistent pair. The hash does not prove origin, identity, authorship, trusted time, or legal signature.
